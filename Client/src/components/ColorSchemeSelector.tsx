...
const ColorSchemeSelector = ({value,onChange}:{value:string;
  ...
  return (
    <div className="space-y-3">
      <label className="block text-smm font-medium text-zinc-200">Color Scheme</label>
      
      <div className="grid grid-cols-6 gap-3">
{colorSchemes.map((scheme)=>(
    <button key={scheme.id}
        onClick={()=>onChange(scheme.id)}
        className={`realtive rounded-lg transition-all $
     ...
     title={scheme.name}>
        ...
        <div className="flex h-10 rounded-lg overflow-hidden">
          {scheme.colors.map((color,i)=>(
             <div
             key={i} className="flex-1" style={{backgroundColor:color}}/>
          ))}
        </div>
     </button>
))}
